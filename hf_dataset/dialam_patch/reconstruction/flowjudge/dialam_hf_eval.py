...
from .dialam_calibration_v5 import calibrated_pairwise_label
from .dialam_training_v5 import assemble_pairwise_patch, build_pairwise_prompt
...
def run_dialam_hf_evaluation(
    *,
    ...
    inference_config = _declared_dialam_inference(model_id)
    pairwise_inference = (
        inference_config.get("schema_version")
        == "dialam_pairwise_inference_v5_1"
    )
    ...
    for role, target_model in targets:
        ...
        with responses_path.open("w", encoding="utf-8") as responses_file:
            for index, example in enumerate(examples, start=1):
                if pairwise_inference:
                    response, pairwise_decisions = _pairwise_prediction(
                        generator,
                        example,
                        inference_config,
                    )
                    prompt_evidence: str | list[dict[str, Any]] = [
                        {
                            "target_id": item["target_id"],
                            "prompt": item["prompt"],
                        }
                        for item in pairwise_decisions
                    ]
                else:
                ...
                prediction = {
                    ...
                    "inference_strategy": (
                        "pairwise_v5_1" if pairwise_inference else "zero_shot"
                    ),
    ...
    manifest = {
        ...
        "prompt_strategy": "pairwise_v5_1" if pairwise_inference else "zero_shot",
        "inference_config": inference_config if pairwise_inference else None,
        "summaries": summaries,
    }
...
def _declared_dialam_base_model(model_id: str) -> str | None:
    for filename in (
        "dialam_v5_checkpoint_manifest.json",
        "dialam_v3_checkpoint_manifest.json",
        "dialam_v1_checkpoint_manifest.json",
    ):
        manifest = _read_json_resource(model_id, filename)
        fixed_config = manifest.get("fixed_config", {})
        value = (
            fixed_config.get("base_model")
...
def _declared_dialam_inference(model_id: str) -> dict[str, Any]:
    return _read_json_resource(model_id, "dialam_inference_config.json")
...
def _pairwise_prediction(
    ...
    labels = tuple(inference_config.get("allowed_labels", ()))
    if labels != ("NONE", "SUPPORT", "ATTACK", "REPHRASE"):
        raise ValueError("v5.1 inference config has unexpected allowed labels")
    margin = float(inference_config.get("none_margin", -1.0))
    if margin < 0:
        raise ValueError("v5.1 inference config needs a nonnegative NONE margin")
    max_sequence_length = int(inference_config.get("max_sequence_length", 2048))
    candidate_ids = [item.id for item in example.earlier_propositions]
    selected_labels = []
    decisions: list[dict[str, Any]] = []
    for candidate_id in candidate_ids:
        prompt = build_pairwise_prompt(example, candidate_id)
        scores = generator.score_completions(
            prompt,
            labels,
    ...
    return (
        assemble_pairwise_patch(example.new_proposition.id, candidate_ids, selected_labels),
        decisions,
    )
