...
class HuggingFaceGenerator:
    ...
    def score_completions(
        ...
        *,
        ...
        """Return mean token log probability for each exact assistant completion."""
        if not completions or len(completions) != len(set(completions)):
            raise ValueError("completions must be a nonempty unique tuple")
        ...
        completion_ids = {
            completion: self.tokenizer(completion, add_special_tokens=False)["input_ids"]
            for completion in completions
        }
        if not prompt_ids or any(not tokens for tokens in completion_ids.values()):
            raise ValueError("prompt or completion tokenization is empty")
        maximum_length = max(
            len(prompt_ids) + len(tokens) for tokens in completion_ids.values()
        )
        if max_sequence_length is not None and maximum_length > max_sequence_length:
            raise ValueError(
                f"completion scoring needs {maximum_length} tokens, exceeding "
                f"{max_sequence_length}"
            )
        pad_id = self.tokenizer.pad_token_id
        if pad_id is None:
            raise ValueError("completion scoring requires a tokenizer pad token")
        sequences = [prompt_ids + completion_ids[item] for item in completions]
        ...
        for index, sequence in enumerate(sequences):
            input_ids[index, : len(sequence)] = self.torch.tensor(
                sequence, device=self.model.device
            )
        ...
        for row_index, completion in enumerate(completions):
            tokens = completion_ids[completion]
            token_scores = [
                log_probs[row_index, len(prompt_ids) + offset - 1, token_id]
                for offset, token_id in enumerate(tokens)
            ]
            scores[completion] = float(
                self.torch.stack(token_scores).mean().item()
            )
        return scores
